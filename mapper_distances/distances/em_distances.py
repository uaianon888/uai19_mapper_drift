...
def naw_distance(net1, net2, metric_space, p=None, q=None):
    """ Wraps lightweight mapper objects to calculate NAW
    from properties
    ...
    gw_dist, params = naw_calc_pot(dists1_short, dists2_short, C3, p, q)
    return .5 * gw_dist, params

def naw_calc_glop(D1, D2, Dp, mu1, mu2, tol=.001, p=1, mode='stable'):
...
def naw_calc_pot(D1, D2, Dp, mu1, mu2):
    ...
    D1/D2 ~ np.array - nxn, mxm
        Distance matrices for each sub-metric space
    Dp ~ np.array - nxm
        Pairwise distances between spaces
    mu1/mu2 - np.array - nx1, mx1
        Discrete measure spaces
    returns:
    ----------
...
def gromov_wasserstein_mapper(net1, net2, metric_space):
    """ Wraps GW distance with the Mapper object.

    Might have found the least efficient way to implement this...
...
def exact_gw(D1, D2, mu1, mu2, tol=.001, p=1, mode='stable'):
    ...
    return .5 * compute_gw_from_transport_sq(results, D1, D2)
